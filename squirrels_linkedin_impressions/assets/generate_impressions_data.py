...
def generate_company_data():
    ...
    company_data = {
        "company_id": [f"C{str(i).zfill(4)}" for i in range(1, num_companies+1)],
...
def generate_user_data():
    ...
    job_titles = ["Software Engineer", "Data Scientist", "Product Manager", "Designer", "Sales Manager", "HR Specialist"]
    city_names = ["New York", "San Francisco", "Chicago", "Los Angeles", "Austin", "Seattle", "Toronto", "Vancouver"]
    ...
    user_data = {
        ...
        "company_id": [f"C{str(random.randint(1, num_companies)).zfill(4)}" for _ in range(num_users)],
...
def generate_impressions_data(start_date: date, end_date: date):
    ...
    impressions_data = {
        "post_id": ["P0001" for _ in range(num_impressions)],
        "date": [start_date + timedelta(days=random.randint(0, days_between)) for _ in range(num_impressions)],
        "user_id": [f"U{str(random.randint(1, num_users)).zfill(4)}" for _ in range(num_impressions)],
    }
    return pd.DataFrame(impressions_data).sort_values("date")
...
impressions_df = generate_impressions_data(date(2024,10,1), date(2024,10,14))
